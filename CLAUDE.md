...
- **Pinned tabs** (Key, 2026-09-19): pin button per drawer row; pinned tabs sit first in pin order, can't be closed until unpinned, and agents must never navigate, reuse or close them. Stored by the server (`/api/pins`, file `~/.cache/thor-viewer-pins.json`, `THOR_PINS_FILE`); matching by tab id then URL is in `public/pins.js` (shared by page and server). `/api/pins/active` says whether the current tab is pinned: the gate turns `open` on a pinned tab into `tab new` (Stream) or refuses it (Live), and refuses `tab close`; `start.sh` reopens missing pins (`POST /api/pins/restore`) and opens its URL in a new tab when the current one is pinned. `/api/shared-tabs` marks each tab `pinned`.
