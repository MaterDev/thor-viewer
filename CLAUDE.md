...
- **Pinned tabs** (Key, 2026-09-19): pin button per drawer row; pinned tabs sit first in pin order, can't be closed until unpinned, and agents must never navigate, reuse or close them. Stored durably in `~/.local/state/thor-viewer/pins.json` (atomic write, `.bak` fallback; `THOR_PINS_FILE`). **Only Key changes pins**: `POST /api/pins` accepts the drawer (same-origin fetch) or `tools/pins` (header `x-thor-pins`), and the Claude Code hook `~/.claude/hooks/guard-pins.py` denies `tools/pins pin|unpin`, pins-API POSTs and pins-file writes unless Key's latest message says "pin"/"unpin". `tools/pins list` is free. matching by tab id then URL is in `public/pins.js` (shared by page and server). `/api/pins/active` says whether the current tab is pinned: the gate turns `open` on a pinned tab into `tab new` (Stream) or refuses it (Live), and refuses `tab close`; `start.sh` reopens missing pins (`POST /api/pins/restore`) and opens its URL in a new tab when the current one is pinned. `/api/shared-tabs` marks each tab `pinned`.
