...
- `~/.local/bin/agent-browser` is the routing gate (`tools/agent-browser-gate`): plain commands follow the mode; in Live they act on Key's page (`@refs` from `snapshot`); `close`/`set`/`session`/`stream` are refused, `tab` goes to the viewer (see above). `--headless` borrows your own page for one command. Scripts that always mean the headless page (tests, start.sh, this server) set `THOR_GATE=off`. Install/remove: `bash tools/install-gate.sh install` / `uninstall` (`status` to check).
...
- **Pinned tabs** (Key, 2026-09-19): pin button per drawer row; pinned tabs sit first in pin order, can't be closed until unpinned, and agents must never navigate, reuse or close them. Stored durably in `~/.local/state/thor-viewer/pins.json` (atomic write, `.bak` fallback; `THOR_PINS_FILE`). **Only Key changes pins**: `POST /api/pins` accepts the drawer (same-origin fetch) or `tools/pins` (header `x-thor-pins`), and the Claude Code hook `~/.claude/hooks/guard-pins.py` denies `tools/pins pin|unpin`, pins-API POSTs and pins-file writes unless Key's latest message says "pin"/"unpin". `tools/pins list` is free. matching by tab id then URL is in `public/pins.js` (shared by page and server). `/api/pins/active` says whether the current tab is pinned: the gate turns `open` on a pinned tab into `tab new` (both modes) and refuses `tab close` for it; `start.sh` reopens missing pins (`POST /api/pins/restore`) and opens its URL in a new tab when the current one is pinned. `/api/shared-tabs` marks each tab `pinned`.
- **One shared tab list** for Stream and Live, owned by the server (`GET/POST /api/shared-tabs`, file `~/.cache/thor-viewer-tabs.json`, `THOR_TABS_FILE` to override). In Stream it is read from the headless tabs; entering Live opens the active tab. **Closing a tab closes the page behind it** (Key, 2026-09-19: no floating streams): in Live the client posts `/api/shared-tabs/closed {url}`, and leaving Live (`POST /api/shared-tabs/to-stream`) syncs the headless tabs to the shared list, closing extras and opening what's missing. Pinned URLs are never closed by a sync. Test: `npm run test:tabs` (isolated instance on 4851, own session/profile; safe while Key is in Live).
