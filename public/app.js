...
function draw() {
  ...
  if (cursor.visible) {
    ...
    ctx.beginPath(); ctx.arc(cursor.x, cursor.y, r, 0, Math.PI * 2); ctx.strokeStyle = '#9dbcff'; ctx.lineWidth = lw; ctx.stroke();
    ctx.beginPath(); ctx.arc(cursor.x, cursor.y, r / 5, 0, Math.PI * 2); ctx.fillStyle = '#9dbcff'; ctx.fill();
  }
}
